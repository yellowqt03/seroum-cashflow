...
export function AllocationCard({
  ...
  const getUsageColor = (rate: number) => {
    ...
    if (rate >= 70) return 'text-slate-900 bg-orange-100'
    if (rate >= 50) return 'text-yellow-600 bg-yellow-100'
    return 'text-slate-900 bg-green-100'
  }
    ...
    <div className="bg-white rounded-lg border border-slate-200 p-6 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-3 mb-4">
        ...
        <div className="flex justify-between items-center text-sm">
          ...
          <span className="font-semibold text-slate-900">{allocation.usedAmount}매</span>
        </div>
        <div className="flex justify-between items-center text-sm">
          <span className="text-gray-600">잔여</span>
          <span className="font-semibold text-slate-900">{allocation.remainingAmount}매</span>
        </div>
      ...
      {allocation.autoRefresh && (
        <div className="flex items-center gap-2 text-xs text-gray-600 mb-4 bg-slate-50 p-2 rounded">
      ...
      {/* 메모 */}
      {allocation.note && (
        <div className="text-xs text-gray-600 mb-4 p-2 bg-slate-50 rounded">
          {allocation.note}
        </div>
      )}
